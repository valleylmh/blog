        ...
        for (const oldSrc of imgMatches) {
            if (!oldSrc.includes('http')) {
                ...
                if (isPreview) {
                    ...
                    // 预览模式下，如果需要浏览器能看到，可以转为 file:// 路径或保留原样（如果是相对路径且 preview.html 在相同位置的话）
                    // 这里为了方便，我们直接把路径修正为相对于当前执行目录的路径，或者直接用绝对路径
        ...
        if (isPreview) {
            ...
            await fs.writeFile(previewPath, `
                ...
                <head>
                    ...
                    <style>
                        body { background: #f0f2f5; padding: 20px; display: flex; justify-center; }
                        .phone-mockup { 
                            background: white; 
                            width: 375px; 
...
publishArticle(
    './docs/ai/articles/date/20260324.md',
    './docs/public/images/articles/20260324/cover.png',
    '时隔五年重启创作表达：我的 OpenClaw “养虾”实验',
    isPreviewMode
);
